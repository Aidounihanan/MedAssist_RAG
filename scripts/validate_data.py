...
for pdf_path in PDF_FILES:
    ...
    try:
        ...
        if words < 50:
            warn(
                f"{fname} — {n_pages} pages, {size_kb:.0f} KB — PDF SCANNÉ (peu de texte extractible : {words} mots)"
            )
            info("→ Sera traité par OCR en Phase 4 — déplacer vers data/03_scans/ si scan pur")
            total_warn += 1
            pdf_results.append(
                {"file": fname, "pages": n_pages, "words_sample": words, "status": "scanned"}
            )
        else:
...
for tbl_path in TABLE_FILES:
    ...
    try:
        ...
        err(f"{fname} — Erreur lecture : {e}")
        info("→ Fichier potentiellement corrompu ou format non standard")
        total_err += 1
    ...
    if pdf_ok:
        try:
            ...
            if words > 30:
                ...
                warn(
                    f"calendrier_vaccination_national.pdf — {n_pages} pages — PDF scanné ou tableaux image"
                )
